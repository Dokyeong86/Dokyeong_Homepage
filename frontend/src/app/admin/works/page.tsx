// Dokyeong_Homepage/frontend/app/admin/works/page.tsx
'use client';
...
export default function AdminWorksPage() {
  ...
  // 신규 등록/수정 모달 열기
  const openModal = (mode: 'create' | 'edit', work?: Work) => {
    setModalMode(mode);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (modalMode === 'edit' && selectedWork) {
        await updateWork(selectedWork.id, form);
      }
      setShowModal(false);
      fetchWorks();
    } catch {
    ...
    >
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full bg-white rounded-2xl shadow-lg overflow-hidden">
              ...
              <tbody>
                {works.map((work) => (
                  <tr key={work.id} className="border-b last:border-none">
                    <td className="py-3 px-4 font-semibold">{work.title}</td>
                    <td className="py-3 px-4 text-gray-700 line-clamp-2 max-w-xs">{work.description}</td>
                    <td className="py-3 px-4 text-gray-400 text-sm">{new Date(work.created_at).toLocaleDateString()}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          ...
          >
            <h2 className="text-2xl font-bold mb-6">{modalMode === 'create' ? '새 작품 등록' : '작품 수정'}</h2>
